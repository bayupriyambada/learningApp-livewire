        ...
        <div class="page-header d-print-none">
            <div class="row align-items-center">
                ...
                <div class="col-auto ms-auto d-print-none">
                    <div class="d-flex">
                        {{-- <select class="form-select d-inline-block w-9 me-3" wire:model="selectedGradeLevel">
                            <option>Pilih Kelasan</option>
    ...
    <div class="page-body">
        <div class="container-xl">
                        ...
                        <div class="card">
                            <div class="card-body p-4 text-center">
                                <span class="avatar avatar-xl mb-3 avatar-rounded"
                                    ...
                                    url('https://ui-avatars.com/api/?name={{ $student->name }}')"></span>
                                <h3 class="m-0 mb-1"><a href="#">{{ $student->name }}</a></h3>
                                <div class="mt-3">
                                    <span class="badge bg-green-lt">NIK. {{ $student->nik }}</span>
                                    <span class="badge bg-red-lt">{{ $student->gradeLevel->grade }}</span>
                                </div>
                            </div>
                            <div class="d-flex">
                                <a href="#" wire:click.prevent="showDetail({{ $student }})" class="card-btn">
                                    Detail</a>
                                <a href="#" wire:click.prevent="showEdit({{ $student }})" class="card-btn">
                                    Edit</a>
                                <a href="#" wire:click.prevent="showDelete({{ $student }})" class="card-btn">
                                    Delete</a>
                            </div>
                            <div class="d-flex">
                                <a href="#" wire:click.prevent="activate({{ $student->id }})"
                                    class="card-btn  @if ($student->is_active === 1) btn btn-dark disabled w-100 hidden @endif">
                                    {{ $student->is_active === 0 ? 'Belum Aktif' : 'Telah Aktif' }}</a>
                            </div>
                        </div>
                    </div>
                @empty
            ...
            <div class="d-flex mt-4">
                {{ $students->links('livewire::simple-bootstrap') }}
            </div>
        </div>
    </div>
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="save()" autocomplete="off">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" wire:model="grade_levels_id">
                                <option>Pilih Kelasan</option>
                                @foreach ($gradeLevels as $item)
                                    <option value="{{ $item->id }}">{{ $item->grade }}</option>
                                @endforeach
